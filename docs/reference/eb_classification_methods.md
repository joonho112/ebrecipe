...
These methods summarize and extract the per-unit FDR classification
table.

## Usage

``` r
# S3 method for class 'eb_classification'
print(x, ...)
...
`summary.eb` list. [`nobs()`](https://rdrr.io/r/stats/nobs.html) returns
the number of tested units, and
[`as.data.frame()`](https://rdrr.io/r/base/as.data.frame.html) returns
the per-unit selection table.

## Details

[`as.data.frame()`](https://rdrr.io/r/base/as.data.frame.html) returns
only the unit-level `p_value`, `q_value`, and `selected` fields.
Frontier summaries and scalar metadata such as `pi0` remain on the
original object.
